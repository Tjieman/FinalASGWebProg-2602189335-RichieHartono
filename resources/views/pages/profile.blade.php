    ...
    <div class="container">
        ...
        <div class="row mt-2 mb-5">
            ...
            <h3 class="text-center">
                @php
                    $username = str_replace('https://www.linkedin.com/in/', '', Auth::user()->linkedInUsername);
                @endphp
                {{ $username }}
            </h3>
        </div>
                                    ...
                                    @auth
                                        @if (Auth::user()->follows($user))
                                            <form action="{{ route('unfollow', $user->id) }}" method="POST">
                                                @csrf
                                                <button type="submit" class="btn btn-success">
                                                    <i class="bi bi-hand-thumbs-up-fill"></i> {{ __('profile.followed') }}
                                                </button>
                                                <button type="submit" class="btn btn-danger">
                                                    <i class="bi bi-hand-thumbs-down-fill"></i> {{ __('profile.unfollow') }}
                                                </button>
                                            </form>
                                            ...
                                            @if (Auth::user()->areMutualFollowers($user))
                                                <a href="{{ route('call', $user->id) }}" class="text-decoration-none">
                                                    <button class="btn btn-info d-flex">
                                                        <i class="bi bi-telephone-fill" style="color: white"></i>
                                                        <span class="ms-2" style="color: white">{{ __('profile.call') }}</span>
                                                    </button>
                                                </a>
                                            @endif
                                        @else
                                            <form action="{{ route('follow', $user->id) }}" method="POST">
                                                @csrf
                                                <button type="submit" class="btn btn-secondary">
                                                    <i class="bi bi-hand-thumbs-up"></i> {{ __('profile.follow') }}
                                                </button>
                                            </form>
                                        @endif
                                    @endauth
